...
import { ArrowLeft, Save, X, Plus, Trash2, Languages } from "lucide-react";
...
export default function NewBillPage() {
  ...
  const { register, control, handleSubmit, setValue, watch, formState: { errors } } = useForm<BillFormValues>({
    defaultValues: {
      vendorId: "",
      ...
      notes: "",
      lines: [
        { productId: "", description: "", qty: 1, unit: "Pcs", unitPrice: 0, vatRate: 15 }
      ]
    }
  });
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        console.error("Failed to load bill context data:", err);
      } finally {
        setLoading(false);
      }
    }
  ...
  const onSubmit = async (values: BillFormValues) => {
    setBusy(true);
    try {
      await api.createBill(values);
      router.push("/purchasing/bills");
    } catch (err: any) {
      alert("Failed to save purchase bill: " + err.message);
    } finally {
      setBusy(false);
    }
  };
  ...
  if (loading) {
    return (
      <PageShell>
        <div className="p-8 text-zinc-350">Loading purchase bill form parameters...</div>
      </PageShell>
    );
  }
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 max-w-6xl mx-auto pb-12">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-zinc-800">
          <div className="space-y-1">
            <div className="flex items-center gap-2 text-xs text-zinc-400">
              ...
              <span>/</span>
              <span className="text-zinc-200">New Bill</span>
            </div>
            <h1 className="text-2xl font-black text-zinc-100 tracking-tight">Create Supplier Bill</h1>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <Save className="h-4 w-4 mr-2" /> {busy ? "Saving Bill..." : "Save Bill"}
            </Button>
          </div>
        </div>
        ...
        <Card className="bg-zinc-900/30 border-zinc-800">
          <CardContent className="p-6 grid sm:grid-cols-4 gap-4">
            ...
            <div className="space-y-2">
              <Label className="text-zinc-350">Supplier Invoice Ref #</Label>
              <Input
                ...
                {fields.map((field, index) => {
                  ...
                  return (
                    <TableRow key={field.id} className="border-zinc-800 hover:bg-transparent">
                      ...
                      {/* Description Input + Translation trigger */}
                      ...
                      <TableCell className="align-top text-center pr-4">
                        <Button
                          ...
                          className="h-9 w-9 text-zinc-450 hover:text-rose-400 hover:bg-zinc-800"
                        >
