        ...
        <div className="flex flex-wrap items-center justify-between gap-4 mb-6 p-4 bg-white rounded-xl shadow-sm border print:hidden no-print">
          <div className="flex items-center gap-3">
            <Button asChild variant="outline" size="sm">
              <Link href="/invoicing/quotations"><ArrowLeft className="h-4 w-4 mr-1" /> Back</Link>
            </Button>
                ...
                <td className="p-0 border-none">
                    ...
                    <div className="flex justify-between items-start pb-8 border-b-2 border-zinc-200">
                      ...
                      <div className="text-right">
                        <div className="bg-zinc-100 rounded-lg p-3 inline-block text-right mb-2">
                          <span className="block text-lg font-black text-zinc-850 uppercase tracking-wider">Sales Quotation</span>
                          <span className="block text-sm font-bold text-zinc-700 tracking-wide dir-rtl mt-0.5">
                            عرض سعر
                          </span>
                        </div>
                        <h2 className="text-4xl font-black text-zinc-900 mt-1">{q.number}</h2>
                        <div className="mt-2 inline-block font-normal">
                          <StatusBadge status={q.status} />
                          {q.status === "invoiced" && q.invoice_number && (
                            <span className="ml-2 text-xs text-muted-foreground block font-semibold mt-1">({q.invoice_number})</span>
                          )}
                        </div>
                      </div>
                    </div>
                  ...
                  {printOnLetterhead && (
                    <div className="flex justify-between items-start pb-6 border-b-2 border-zinc-200">
                      <div>
                        <span className="block text-2xl font-black text-zinc-850 uppercase tracking-wider">Sales Quotation</span>
                        <span className="block text-lg font-bold text-zinc-700 tracking-wide dir-rtl mt-0.5">
                          عرض سعر
                        </span>
                      </div>
                      <div className="text-right">
                        <h2 className="text-4xl font-black text-zinc-900">{q.number}</h2>
                  ...
                  <div className="grid grid-cols-3 gap-8 py-8 text-sm">
                    <div>
                      <p className="text-[10px] uppercase font-bold tracking-wider text-zinc-400 mb-2">Quote To / العميل</p>
                      <div className="space-y-1">
                        <p className="font-bold text-base text-zinc-800">{q.customer_name ?? "—"}</p>
                        {q.customer_vat && (
                          <p className="text-xs text-zinc-500 font-medium">
                            <span className="text-zinc-400">VAT:</span> {q.customer_vat}
                          </p>
                        )}
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="space-y-1.5">
                        <div className="flex justify-between text-xs">
                          ...
                          <span className="font-semibold text-zinc-700">{formatDate(q.issue_date)}</span>
                        </div>
                        {q.due_date && (
                          <div className="flex justify-between text-xs">
                            <span className="text-zinc-400">Expiry Date:</span>
                            <span className="font-semibold text-zinc-700">{formatDate(q.due_date)}</span>
                          </div>
                        )}
                      </div>
                    </div>

                    {q.custom_fields && Object.keys(q.custom_fields).length > 0 && (
                      <div className="bg-zinc-50/70 rounded-lg p-4 border border-zinc-100 print:bg-transparent print:border-none print:p-0">
                        ...
                        <div className="space-y-1.5 text-xs">
                          {Object.entries(q.custom_fields).map(([key, val]: any) => (
                    ...
                    <table className="w-full text-sm border-collapse">
                      <thead>
                        <tr className="bg-zinc-800 text-white rounded-lg overflow-hidden">
                          ...
                          {isTaxQuote && <th className="text-right p-3 font-semibold text-xs uppercase tracking-wider w-20">VAT / الضريبة</th>}
                      ...
                      <tbody className="divide-y divide-zinc-200">
                        {q.lines?.map((l: any) => (
                          <tr key={l.id} className="hover:bg-zinc-50/50 transition-colors print:break-inside-avoid">
                            ...
                            {isTaxQuote && <td className="p-4 text-right text-zinc-600 font-semibold">{l.vat_rate}%</td>}
                  ...
                  <div className="grid grid-cols-12 gap-8 pt-8 mt-6 border-t-2 border-zinc-100 print:break-inside-avoid">
                    <div className="col-span-7">
                      {/* Left side empty for quotations (non-fiscal) */}
                    </div>
                    ...
                    <div className="col-span-5 space-y-2 text-sm bg-zinc-50/50 p-6 rounded-xl border border-zinc-100 print:bg-transparent print:border-none print:p-0">
                      <div className="flex justify-between py-1">
                        ...
                        <span className="font-bold text-zinc-700">{formatSAR(q.subtotal)}</span>
                      </div>
                      {isTaxQuote && (
                        <div className="flex justify-between py-1">
                          <span className="text-zinc-400 font-medium">VAT 15% / ضريبة القيمة المضافة</span>
                          <span className="font-bold text-zinc-700">{formatSAR(q.vat_total)}</span>
                        </div>
                      )}
                      <div className="flex justify-between items-center text-lg font-black pt-4 border-t border-zinc-200 text-zinc-800">
                        ...
                        <span className="text-xl text-zinc-900">{formatSAR(q.total)}</span>
                      </div>
                    </div>
                  </div>
